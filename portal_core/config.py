...
def print_config(config: SoilGuardConfig):
    ...
    logger.info(f"  Moisture (%)  : {config.thresholds.moisture_min} - {config.thresholds.moisture_max}")
    ...
    logger.info(f"  Soil pH range : {config.thresholds.ph_min} - {config.thresholds.ph_max}")
    logger.info("=" * 60)
